...
import sys
import os
from LLMTools_SeeClass import See  
dotenv.load_dotenv()
...
def get_picture():
    """
    Get a picture from the camera, return it as png file opened with PIL.Image.open(path).
    
    Returns:
        PIL.Image: The image from the camera
        

    """
    img = PIL.Image.open(see.see())
    return img
...
def get_state_of_mind():
    ...
    Returns:
        str: The current state of mind of the user
    """
    return "SO_BAD"
