"""Notification tools for sending messages to user devices."""
...
@tool
def send_notification(headline: str, summary: str, content: str, notification_type: str) -> str:
    ...
    headline (str, required):
        - Brief, descriptive title for the notification (max 250 characters)
    ...
    THE CONTENT MUST BE A WELL FORMATTED MARKDOWN DOCUMENT, NOT A PLAIN TEXT, NOT SOME WEIRD PYTHON STRING BLOCK BUT MARKDOWN DOCUMENT.
    """
    ...
    # Length validation (arbitrary choice, can be changed later)
    max_length = 500

    if len(headline) > max_length * 0.5:
        return "Headline is too long, please provide a headline that is less than 250 characters."
    if len(summary) > max_length:
        return "Summary is too long, please provide a summary that is less than 500 characters."
    if len(notification_type) > max_length * 0.1:
        return "Type is too long, please provide a type that is less than 50 characters."

    state = EchonotiWrapper.send_notification(
        headline=headline,
        summary=summary,
        content=content,
